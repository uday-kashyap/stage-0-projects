...
class WeatherApp(Tk):
    ...
    def load_panels(self) -> None:
        self.feature_panel = Frame(
            self, bg="lightgrey", borderwidth=2, relief=SOLID, width=50
        )
        ...
        self.info_layout = Frame(
            self.info_panel, bg="SystemButtonFace", relief=SUNKEN, borderwidth=2
        )
        self.info_layout.pack(fill=BOTH, expand=True, padx=10, pady=10)

    def load_feature_icons(self) -> None:
        ...
        self.weather_button = Button(
            ...
            command=lambda: self.set_mode("Weather"),
        )
        ...
        self.plot_button = Button(
            ...
            command=lambda: self.set_mode("Forecast"),
        )
    ...
    def weather_info_ui(self, status: str, error_message=None):
        ...
        def customize_temperature_font(temp_val, row_pos, col_pos=1):
            if temp_val >= 30:
                Label(
                    ...
                    font="lucida 10 bold",
                    fg="red",
                ).grid(row=row_pos, column=col_pos)
            elif temp_val > 25:
                Label(
                    self.info_layout,
                    text=f"{temp_val}°C",
        ...
        try:
            if status == "Success":
                ...
                # Set and display the information on UI
                Label(
                    self.info_layout, text="Current Weather:", font="lucida 10 bold"
                ).grid(row=0, column=0, sticky="w")
                customize_temperature_font(self.temperature, row_pos=0)
                Label(self.info_layout, text="Condition:", font="lucida 10 bold").grid(
                    row=1, column=0, sticky="w"
                )
                Label(
                    self.info_layout, text=f"{self.condition}", font="agencyfb 10 bold"
                ).grid(row=1, column=1)
                Label(
                    self.info_layout, text="Feels Like:", font="lucida 10 bold"
                ).grid(row=2, column=0, sticky="w")
                customize_temperature_font(self.feels_like, row_pos=2)
                Label(self.info_layout, text="Humidity:", font="lucida 10 bold").grid(
                    row=3, column=0, sticky="w"
                )
                Label(
                    self.info_layout, text=f"{self.humidity}%", font="agencyfb 10 bold"
                ).grid(row=3, column=1)
    ...
    def forecast_choices_ui(self):
        def fetch_temperature_plot():
            ...
            try:
                self.forecast.plot_forecast(
                    city=self.city_name.get(), type="temperature"
                )
            except Exception as e:

                # Handle errors
                error_message = buffer.getvalue().strip()
                Label(self.info_layout, text=error_message).pack()
        ...
        def fetch_humidity_plot():

            # Clear previous content
            self.clear_info_layout()

            try:
                self.forecast.plot_forecast(city=self.city_name.get(), type="humidity")
            except:
        ...
        plot_temp_button = Button(
            self.info_layout, text="Plot Temperature", command=fetch_temperature_plot
        )
        plot_temp_button.pack(pady=10)
        plot_humidity_button = Button(
            self.info_layout,
            text="Plot Humidity",
            width=13,
            command=fetch_humidity_plot,
        )
        plot_humidity_button.pack()
    ...
    def get_info(self):
        ...
        if self.active_mode == "Weather":
            ...
            if weather_data:
                ...
                self.weather_info_ui(
                    status="Error", error_message=buffer.getvalue().strip()
                )
        else:
            self.forecast = Forecast(API_KEY)
            self.forecast_choices_ui()
    ...
    def display_searchbox(self):
        ...
        self.city_searchbox = Entry(
            self.searchbox_panel,
        ...
        self.search_button = Button(
            self.searchbox_panel, text=f"Get {self.active_mode}", command=self.get_info
        )
